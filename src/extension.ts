...
export function activate(context: vscode.ExtensionContext) {
  ...
  // register command to set image folder (absolute path)
...
function showPanel(context: vscode.ExtensionContext, folderPath: string) {
  // determine base folder: absolute or default
  ...
  // try reading provided folder; if empty or no png, fallback to default
  let pics: string[] = [];
  try {
    pics = fs.readdirSync(imgFolder).filter(f => f.endsWith('.png'));
  } catch {}
  ...
  {
    enableScripts: false,
    localResourceRoots: [vscode.Uri.file(imgFolder)] // English comment: allow loading from custom folder
  }
);
...
<html>
  <body style="margin:0;padding:0;display:flex;flex-direction:column;justify-content:center;align-items:center;">
    ...
    <div style="margin-top:8px;font-size:14px;color:#888;">${pick}</div>
  </body>
</html>`;
}
